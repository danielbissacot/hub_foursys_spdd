...
Qualquer botão, ou icone pode servir para chamar o estado de foco, basta deixa-lo definido como gatilho ativador.
...
id="filter"
data-search="trigger"
class="brad-text-field-search brad-text-field-search--view brad-text-field-search--trigger-off brad-m-md-b no-content"
>
<div class="search-container">
  <label for="search-input" class="brad-text-field brad-m-md-b">
    <input
      id="search-input"
    ...
    <button
      aria-label="Buscar"
      class="text-button trailing-button-text complements"
    >
      Buscar
    </button>
...
<div
  class="search-view-options search-view-options__hide brad-p-lg-x brad-m-lg-b"
>
  <button
    role="option"
...
```
## Como o data-btn-type="text-field-search" funciona

O atributo data-btn-type="text-field-search" é crucial para ativar a view de busca ao ser clicado. Esse atributo identifica o botão ou ícone que serve como gatilho para abrir o campo de pesquisa. Quando o botão é clicado, o serviço BradTextFieldSearchService é acionado para exibir o componente de busca correspondente, com base no tipo de busca configurado (no caso do tipo trigger, a view será aberta em tela cheia).

Esse comportamento ocorre automaticamente quando o serviço é inicializado e o botão com o atributo data-btn-type="text-field-search" é clicado, disparando a exibição da view de pesquisa definida no HTML.

Alternativa importante: Além de usar o atributo data-btn-type="text-field-search", você também pode chamar o método open() diretamente para abrir a view de busca. Isso permite que o comportamento de pesquisa seja ativado por lógica de código, sem necessidade de interação com o UI.
...
✅ Nesta documentação, o tipo demonstrado é o trigger, no qual a view em tela cheia é ativada por um elemento de gatilho, e ao selecionar uma opção, a view se fecha e o usuário retorna ao estado anterior.
...
Além de utilizar o botão com o atributo data-btn-type="text-field-search" como gatilho, o serviço BradTextFieldSearchService também permite abrir a view de busca de forma programática utilizando o método open(). Isso proporciona maior flexibilidade, permitindo que a busca seja acionada diretamente via código, sem depender da interação com o botão específico.
  ...
  >
    <div class="search-container">
      ...
      >
        ...
        <button
          aria-label="Buscar"
          class="trailing-button-text complements text-button"
        >
          Buscar
        </button>
