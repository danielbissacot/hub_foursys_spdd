...
| containingBlockSelector(opcional) | string | - | Seletor CSS do ancestral que cria um novo containing block para elementos com position: fixed, como modais com transform. Use para compensar o offset em superfícies sobrepostas. |
...
Use containingBlockSelector quando o drag and drop estiver dentro de um ancestral com transform que afete o posicionamento de elementos com position: fixed, como brad-modal centralizado.
...
Dentro de brad-modal ou qualquer overlay centralizado com transform.
Em componentes customizados que apliquem transform no container do drag and drop.
Também no useModeCollapse, se esse modo estiver dentro de um ancestral transformado.

# Quando não usar:

Em páginas comuns e listas sem ancestral transformado.
Em brad-bottom-sheet e brad-side-sheet, que não precisam dessa compensação.
Em cenários onde o card já está sendo posicionado corretamente sem deslocamento.

## Templates prontos para esses cenários:
...
O drag and drop agora tenta identificar automaticamente o ancestral scrollável mais próximo. Isso cobre cenários como brad-modal__content, brad-bottom-sheet__content e brad-side-sheet__content, priorizando o container que realmente possui overflow e conteúdo rolável.

A classe css "brad-drag-and-drop-scroll" continua disponível como fallback legado e também pode ser usada quando você quiser forçar explicitamente qual elemento deve realizar o scroll durante o arraste. No exemplo desta documentação, a classe foi colocada na tag "html", ou seja, ao realizar o drag de um elemento e arrastar no começo ou no final da tela a página principal realizará o scroll. A classe pode ser colocada também em outros elementos como div's desde que estes elementos sejam responsáveis pelo scroll.
...
Quando o card precisar apresentar um conteúdo diferente no estado colapsado, forneça uma versão resumida para o colapso e outra completa para o estado expandido. Isso é indicado quando o card expandido tem descrição longa, tags, ações ou múltiplas linhas de informação. Se o layout já for compacto, o mesmo conteúdo pode ser reaproveitado nos dois estados com pequenas adaptações.
...
Como passar o conteúdo no modo colapso: coloque a versão resumida em brad-drag-and-drop__content-collapsed e a versão completa em brad-drag-and-drop__content-expanded. Isso é necessário quando o card expandido possui descrição longa, tags, ações ou múltiplas linhas de informação. Se o card já for compacto, os dois blocos podem reaproveitar praticamente a mesma estrutura, variando apenas o nível de detalhe.

```
<div class="brad-drag-and-drop__content" id="card-1">
<div class="brad-drag-and-drop__content-collapsed brad-card brad-card--default brad-p-lg">
  <p>Título do card</p>
</div>

<div class="brad-drag-and-drop__content-expanded brad-card brad-card--default brad-p-lg">
  <p>Título do card</p>
...
<section>
  <p class="info-a11y" aria-live="assertive"></p>
  <div id="dragAndDrop-collapse-114" class="brad-drag-and-drop">
    <div
      name="Collapse Mode"
