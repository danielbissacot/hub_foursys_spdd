...
```
## Uso em superfícies sobrepostas

Os cenários novos de modal, bottom sheet e side sheet passam a ter comportamentos diferentes conforme a superfície hospedeira:

brad-modal: quando o modal usa transform, a compensação de offset depende de containingBlockSelector no serviço HTML.
brad-bottom-sheet e brad-side-sheet: funcionam sem configuração extra, porque não precisam dessa compensação.
Scroll durante o drag: o componente agora prioriza automaticamente o ancestral scrollável mais próximo quando há um container com overflow real.

Importante: o WebComponent não expõe um atributo específico para containingBlockSelector. Para cenários com modal centralizado e transform, prefira os templates prontos ou a inicialização via serviço HTML.
...
Como passar o conteúdo no modo colapso: declare a versão compacta dentro de brad-drag-and-drop-content-collapsed e a versão completa dentro de brad-drag-and-drop-content-expanded. Isso é necessário quando o card expandido tem descrição longa, tags, ações ou múltiplas linhas. Se o card já for compacto, os dois blocos podem reaproveitar praticamente a mesma estrutura, mudando apenas o nível de detalhe.
...
<section>
  <p class="info-a11y" aria-live="assertive"></p>
  <brad-drag-and-drop id="drag-and-drop-collapse-97" brad-use-mode-collapse>
    <brad-drag-and-drop-container
      name="Favoritos"
