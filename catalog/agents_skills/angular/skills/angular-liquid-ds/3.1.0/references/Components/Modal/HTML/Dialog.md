...
O método getPreviouslyFocusedElement() do componente modal retorna o último elemento focado antes da abertura, também servindo de opção para o focus, como no exemplo a seguir:

```
const targetSelector = "#dialog";
const options = { targetSelector };
const service = LiquidCorp.BradModalService.getInstance(options);
const modalElement = service.getElementEvent();

modalElement.addEventListener("close", () => {
service.getPreviouslyFocusedElement()?.focus();
});
```
Exemplo
Modal dialog
...
>
  <div class="brad-modal--dialog__top">
    <div class="brad-modal--dialog__header"><h2>Title-Lorem ipsum dolor sit</h2></div>
    <div class="brad-modal--dialog__paragraph"><p>Paragraph-Lorem ipsum dolor sit amet, consectetur adipiscing elit. Nullam eget
    aliquet massa. Fusce vestibulum feugiat massa ac ornare.</p></div>
  </div>
  <div class="brad-modal--dialog__content brad-p-lg"><p><p>Lorem ipsum dolor sit amet...</p></p></div>
  <div
    class="brad-modal--dialog__footer brad-flex brad-flex-row brad-flex-justify-content-end brad-flex-align-items-center"
  >
    <button
