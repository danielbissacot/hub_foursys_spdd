...
# Importante
Este componente é construído sobre a biblioteca Tabulator, utilizando a versão 6.4 como base para renderização e manipulação de dados tabulares.
  ...
  columns: [
    ...
    },
    {
      ...
      titleFormatterParams: {
        ...
        buttonPopover: {
          iconClass: "icon-feedback-view-off",
        },
      },
      resizable: false,
    },
...
LiquidCorp.BradTableService.getInstance(options).then((service) => {
...
LiquidCorp.BradTableService.getInstances(multipleOptions).then((services) => {
...
LiquidCorp.BradTableService.buttonPopoverHeader cria o botão popover para ser utilizado no header. Esse formatador deve ser utilizado no parâmetro titleFormatter. O conteúdo e aparência do popover são configurados via titleFormatterParams com os seguintes parâmetros:


| Parâmetro | Tipo | Obrigatório | Descrição |
| --- | --- | --- | --- |
| titlePopover | string | Não | Título exibido no popover |
| textPopover | string | Não | Texto de conteúdo exibido no popover |
| buttonPopover.iconClass | string | Não | Classe do ícone do botão. Padrão: icon-feedback-view-off |
| buttonPopover.colorClass | string | Não | Classe de cor aplicada ao ícone do botão |
  ...
  columns: [
    {
      ...
      titleFormatterParams: {
        ...
        textPopover: "O texto do popover.",
        buttonPopover: {
          iconClass: "icon-action-info",
          colorClass: "brad-color-primary",
        },
      },
    },
  ],
...
```
## addIconExpansiveLine

LiquidCorp.BradTableService.addIconExpansiveLineTitle e LiquidCorp.BradTableService.addIconExpansiveLine criam um botão com o ícone de seta no título e em cada coluna. LiquidCorp.BradTableService.addExpansiveLine(row) cria a linha expansível com conteúdo customizado para cada row da tabela que será expandida através da seta anteriormente criada.
...
LiquidCorp.BradTableService.addCheckbox quando usado no formatter cria um checkbox para cada linha da coluna de checkbox e no titleFormatter (opcional)* vai criar um checkbox para o Header tendo o modo indeterminate. Logo abaixo um exemplo de utilização.
...
Default
```
<div id="table-348" class="brad-table"></div>
```
Copy code
Action Bar With Selected Rows Counter
```
<div class="brad-flex brad-flex-column">
  <div id="table-97" class="brad-table"></div>
  <div
...
```
<div class="brad-flex brad-flex-column">
  <div id="table-304" class="brad-table"></div>
  <div
    class="brad-flex brad-flex-wrap brad-flex-justify-content-between brad-flex-align-items-center brad-bg-color-cta brad-p-xl"
  >
...
```
<div class="brad-flex brad-flex-column">
  <div id="table-43" class="brad-table"></div>
  <div
    class="brad-flex brad-flex-wrap brad-flex-justify-content-between brad-flex-align-items-center brad-bg-color-cta brad-p-xl"
  >
    <p
