...
Teste aplicações Angular v21+ com Vitest (recomendado) ou Jasmine, com foco em componentes baseados em signals e padrões modernos.

## Configuração do Vitest (Angular v21+)

O Angular v21+ tem suporte experimental para Vitest. É mais rápido e oferece melhor experiência ao desenvolvedor.
...
### Testando signals em componentes
...
## Testando componentes OnPush

Componentes com `OnPush` requerem detecção de mudanças explícita:
...
### Mockando Signal-Based Services
...
## Vitest vs Jasmine — Comparação

| Recurso | Vitest | Jasmine/Karma |
|---------|--------|---------------|
| Velocidade | Mais rápido (ESM nativo) | Mais lento |
| Modo watch | Feedback instantâneo | Reconstruções mais lentas |
| Mocking | `vi.fn()`, `vi.mock()` | `jasmine.createSpy()` |
| Asserções | `expect()` (estilo Chai) | `expect()` (Jasmine) |
| UI | Modo UI integrado | Navegador Karma |
| Config | `vite.config.ts` | `karma.conf.js` |

### Migration de Jasmine para Vitest
