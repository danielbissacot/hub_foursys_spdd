...
Antes de avaliar a história, procure no projeto COBOL aberto se já existem programas, copybooks, JCLs ou variáveis relacionados ao domínio da história (busque por nome de domínio/entidade parecido nos fontes existentes, não só no texto da história). Liste o que encontrar, com caminho real. Use isso como base da especificação técnica — só proponha programa/copybook novo para o que a busca não encontrar.

**Checagem obrigatória da convenção real de nomes**: identifique o prefixo/código real usado nos `PROGRAM-ID` e nomes de copybook já existentes no projeto (muitas organizações usam um prefixo de 2-4 letras ligado ao sistema/empresa). Compare com qualquer prefixo citado em exemplo de skill — nunca use um prefixo de exemplo como se fosse a convenção real deste projeto. O contexto traz um bloco **MAPA REAL DO PROJETO** (identificador base + pastas existentes) — use ele como fonte da verdade. Se o mapa não vier, NÃO trave a fase: gere normalmente, marcando no topo "⚠️ Estrutura proposta sem confirmação contra o projeto — validar antes de implementar".
