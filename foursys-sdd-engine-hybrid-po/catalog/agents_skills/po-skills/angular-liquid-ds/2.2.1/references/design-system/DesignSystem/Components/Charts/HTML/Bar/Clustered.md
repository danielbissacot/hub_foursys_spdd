...
O Chart Bar Clustered é construído através de elementos HTML que trabalham em conjunto para criar a visualização de dados agrupados.
...
Usando async/await
```
const labels = ["Janeiro", "Fevereiro", "Março", "Abril", "Junho"];
const itens = [
{
  barValueLabel: "DataSet1",
  supportingText: "Valor referente ao DataSet1",
...
itens: itens,
type: "vertical-bar-chart-clustered",
...
| type | string | - | Obrigatório. Tipo: "vertical-bar-chart-clustered" ou "horizontal-bar-chart-clustered" |
...
| barWidth | number | 48 | Largura de cada barra no cluster (pixels) |
...
Cada item do array itens representa um dataset (grupo de barras):
...
Para que as barras do agrupamento não fiquem sobrepostas foi criado um método que reorganiza as barras. Para que isso funcione a funcionalidade autoResponsive deve ser ativada.
...
# Exemplos
## Bar Chart Clustered Vertical

## Exemplo básico de gráfico de barras agrupadas vertical:

```
<div id="brad-chart-207" class="brad-chart">
      <div class="brad-chart-wrap">
      ...
      class="brad-btn brad-btn-icon brad-btn-icon--on-color left"
      onclick="window['serviceChart_416'].prevPagination()"
    >
      <em class="btn-icon i icon-ui-chevron-left"></em>
    </button>
<div class="brad-pagination"></div>
<button
      ...
      class="brad-btn brad-btn-icon brad-btn-icon--on-color right"
      onclick="window['serviceChart_416'].nextPagination()"
    >
      <em class="btn-icon i icon-ui-chevron-right"></em>
    </button>
        </div>
...
```
## Bar Chart Clustered Horizontal

Para orientação horizontal, troque o tipo para horizontal-bar-chart-clustered:

```
<div id="brad-chart-152" class="brad-chart">
      <div class="brad-chart-wrap">
...
targetSelector: "#chart",
itens: itens,
type: "horizontal-bar-chart-clustered",
xAxisLabel: "Vendas (R$)",
yAxisLabel: "Meses",
moneyInTooltip: true,
});
```
## Bar Chart Clustered com Paginação
