...
- Resumo funcional da feature a partir de `feature.md` (nome, descrição, regras, critérios de aceite)
...
**Artefatos intermediários** são escritos em `<centro-de-custo>-doc-<projeto>/EPIC-<JIRA_KEY>/_intermediarios/tl/` e servem como handoff entre fases:
```
...
> Os arquivos em `_intermediarios/tl/` são artefatos de trabalho do processo de refinamento. Podem ser consultados para debugging ou reutilizados em futuras iterações.

## Processo

### Passo 1: Coletar contexto da feature

> O acesso à API Jira está bloqueado. Utilize o arquivo `feature.md` como insumo principal.

Leia o arquivo `feature.md` na pasta da feature (`EPIC-<JIRA_KEY>/feature.md`).

Extraia: **nome**, **descrição**, **regras de negócio**, **requisitos** e **critérios de aceite**.
3. Verifique se existe `<centro-de-custo>-doc-<projeto>/EPIC-<JIRA_KEY>/_intermediarios/tl/exploracao.md`:
...
1. Verifique se existe `<centro-de-custo>-doc-<projeto>/workspace-mapping.md` **e** `<centro-de-custo>-doc-<projeto>/EPIC-<JIRA_KEY>/_intermediarios/tl/feature-impact.md`:
...
1. **Para cada serviço impactado, despache um sub-agente** com a seguinte tarefa:
   ...
   - Escrever a análise completa em `<centro-de-custo>-doc-<projeto>/EPIC-<JIRA_KEY>/_intermediarios/tl/analise/<nome-servico>.md`
...
1. **Despache um sub-agente** para executar a skill `usecase-analyzer` nos serviços impactados:
   ...
   - Escrever análises por serviço em `<centro-de-custo>-doc-<projeto>/EPIC-<JIRA_KEY>/_intermediarios/tl/analise/<nome-servico>.md`
...
- Exploração técnica (se existir): `<centro-de-custo>-doc-<projeto>/EPIC-<JIRA_KEY>/_intermediarios/tl/exploracao.md`
- Mapeamento de serviços: `<centro-de-custo>-doc-<projeto>/workspace-mapping.md`
- Impacto na feature: `<centro-de-custo>-doc-<projeto>/EPIC-<JIRA_KEY>/_intermediarios/tl/feature-impact.md`
- Análises por serviço: `<centro-de-custo>-doc-<projeto>/EPIC-<JIRA_KEY>/_intermediarios/tl/analise/*.md`
- Padrões aplicáveis: `<centro-de-custo>-doc-<projeto>/EPIC-<JIRA_KEY>/_intermediarios/tl/padroes-aplicaveis.md`
...
2. **Gerar plano de implementação (após user stories geradas):**
   ...
   > Os planos serão gerados em `specs/<US-slug>/` dentro de cada repositório de serviço impactado.
...
3. **Executar implementação (após planos gerados):**
   ...
   > Os planos são lidos de `../<service-repo>/specs/<US-slug>/` em cada repositório de serviço.
```
