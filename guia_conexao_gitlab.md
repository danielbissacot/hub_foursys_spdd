...
> **"Olá, Assistente de IA! Por favor, configure este microserviço para usar o AI Governance Hub com estes 3 passos: 1. No terminal, execute: `Remove-Item -Path ai-rules, .temp -Recurse -Force -ErrorAction SilentlyContinue; mkdir ai-rules; git clone --branch hub-ia-arquitetura --depth 1 https://oauth2:Z5H2fDfprUFTJKyriWzy@gitlab.fourcamp.com/daniel.bissacot/ai-governance-hub.git .temp; Copy-Item -Path .temp/catalog/* -Destination ai-rules/ -Recurse -Force; git rm -r --cached ai-rules -ErrorAction SilentlyContinue; Remove-Item -Path .temp -Recurse -Force; $m='# 🗺️ Mapa de Ativos: AI Governance Hub`n`nEste arquivo ajuda você a encontrar os prompts certos.`n`n## 📂 Categorias (Use # no chat):`n`n### 1. Refinamento`n- #ESTORIA_USUARIO.md`n- #REQUISITOS_ESTRUTURADOS.md`n`n### 2. Regras Técnicas`n- #REGRAS_NEGOCIO_DOMAIN.md`n`n### 3. Validação`n- #VALIDACAO_HEXAGONAL.md`n- #CODE_REVIEW_SPRING.md`n`n### 4. Entrega`n- #RELEASE_NOTES.md`n- #GIT_COMMIT_PATTERN.md`n`n### 5. Legado (COBOL)`n- #AGENTE_REVERSA_COBOL.md`n- #ENGENHARIA_REVERSA_COMPLETA.md`n`n### 🤖 Agentes`n- #MENTOR_ARCHITECT_SPRING_HEXAGONAL.md`n- #MENTOR_ARCHITECT_ANGULAR.md'; $m | Set-Content -Path MAPA_DO_HUB.md; code MAPA_DO_HUB.md`. 2. ROBÔ GITLAB: Crie o arquivo `.gitlab-ci.yml` na raiz com o conteúdo oficial do Hub. 3. Me avise quando terminar para eu usar o `#`."**
>
> ```yaml
> stages:
>   - validate
...
```powershell
Remove-Item -Path ai-rules, .temp -Recurse -Force -ErrorAction SilentlyContinue; mkdir ai-rules; git clone --branch hub-ia-arquitetura --depth 1 https://oauth2:Z5H2fDfprUFTJKyriWzy@gitlab.fourcamp.com/daniel.bissacot/ai-governance-hub.git .temp; Copy-Item -Path .temp/catalog/* -Destination ai-rules/ -Recurse -Force; git rm -r --cached ai-rules -ErrorAction SilentlyContinue; Remove-Item -Path .temp -Recurse -Force; $m='# 🗺️ Mapa de Ativos: AI Governance Hub`n`nEste arquivo ajuda você a encontrar os prompts certos.`n`n## 📂 Categorias (Use # no chat):`n`n### 1. Refinamento`n- #ESTORIA_USUARIO.md`n- #REQUISITOS_ESTRUTURADOS.md`n`n### 2. Regras Técnicas`n- #REGRAS_NEGOCIO_DOMAIN.md`n`n### 3. Validação`n- #VALIDACAO_HEXAGONAL.md`n- #CODE_REVIEW_SPRING.md`n`n### 4. Entrega`n- #RELEASE_NOTES.md`n- #GIT_COMMIT_PATTERN.md`n`n### 5. Legado (COBOL)`n- #AGENTE_REVERSA_COBOL.md`n- #ENGENHARIA_REVERSA_COMPLETA.md`n`n### 🤖 Agentes`n- #MENTOR_ARCHITECT_SPRING_HEXAGONAL.md`n- #MENTOR_ARCHITECT_ANGULAR.md'; $m | Set-Content -Path MAPA_DO_HUB.md; code MAPA_DO_HUB.md; echo "✅ Hub Conectado com Sucesso! (Mapa Gerado e Aberto)"
```
...
* **Mapa de Ativos (`MAPA_DO_HUB.md`)**: Um arquivo gerado automaticamente com o índice de todos os prompts disponíveis para facilitar a sua navegação.
