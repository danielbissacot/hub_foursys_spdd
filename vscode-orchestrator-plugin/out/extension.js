...
function activate(context) {
    const sidebarProvider = new sidebar_provider_1.HubSidebarProvider(context);
...
async function executePhase(agentName, inputContextFile, phaseName, expectedOutputExtension, context) {
    ...
    if (instructionPath)
        outputChannel.appendLine(`[Instrução Carregada de: ${path.basename(instructionPath)}]`);
    ...
    try {
        const generatedContent = await ai_client_1.AIClient.sendPromptToCopilot(agentName, finalPrompt, finalRules, projectContext, hasMockup, async (chunk) => {
 ...
 */
function removeDeadImports(filePath, rootPath) {
    try {
